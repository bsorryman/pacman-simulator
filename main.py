...
# 메인화면으로 돌아간 후 게임 재시작시 필요한 맵 구조 재선언
...
N_POPULATION = 16  # population에 포함될 chromosome의 개수7
N_BEST = 4  # 상위 우성유전자
...
class Pacman(Figure):
    ...
    def ghost_output(self, genome):
        ...
        '''
        if test == 0: #벽에 막혀있지 않을때
            outputs = np.argmax(outputs)
        elif test == 1: #벽에 막혀있을 때
            max_value_index = np.argmax(outputs) 
            outputs[max_value_index] = 0
            outputs = np.argmax(outputs)
        '''
...
def pointSet():
    ...
    if dlt == True:
        ...
        point_d = {}
        for i, number in enumerate(gamegrid_restart):  # gamegrid 배열을 for문으로 탐색
...
score = 0
point_d = pointSet()
# pacman = Pacman('pacman', (336, 564), genome)  # 2번째 파라미터는 좌표
...
main1 = pygame.image.load('Images\Main_image1.png')
main2 = pygame.image.load('Images\Main_image2.png')
...
if __name__ == "__main__":
    f = open('PlayerOutput.csv', 'w', newline='')
    wr = csv.writer(f)
